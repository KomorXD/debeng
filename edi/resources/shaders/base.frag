...
float point_shadow_factor(int light_idx, vec3 L, float radius) {
    ...
    float randomAngle = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233)))
                              * 43758.5453) * 6.2831853;
    mat2 rotation = mat2(cos(randomAngle), -sin(randomAngle),
                         sin(randomAngle),  cos(randomAngle));

    float shadow = 0.0;
    float sample_radius = 0.005 * u_soft_shadow_props.props.offset_radius / 3.0;
    for (int i = 0; i < 16; i++) {
        vec2 off = rotation * poisson_disk[i];
        vec3 sample_dir = normalize(dir +
                                    off.x * right * sample_radius +
                                    off.y * up * sample_radius);

        shadow += texture(u_point_lights_shadowmaps,
                          vec4(sample_dir, light_idx), curr_depth);
    }
